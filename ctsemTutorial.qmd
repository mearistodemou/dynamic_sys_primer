...
## Linear Change
Let us start by considering a hypothetical patient named Jill. Jill started going to couples therapy and has agreed to track her affect once every seven days (i.e. at equidistant time intervals), for 21 weeks, through a mobile app. What we want, is to conceptualize possible ways that Jill's affect develops during this time. A model in which nothing changes would be the simplest possible baseline. To begin with a minimally dynamic idea, however, we suppose that Jill's affect increases at a steady rate throughout the course of couples therapy.
...
Here we see our linear model provides a good representation of the idea that Jill's affect increases at a steady pace each time we observe her. Albeit with some random fluctuations, which in this simple model we attribute to measurement error rather than to changes in the underlying affect process.
