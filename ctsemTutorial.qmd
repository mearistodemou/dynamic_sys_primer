...
In this tutorial we first focus on developing intuition for continuous-time dynamic systems modelling by building up complexity with a running example, then later address model evaluation and improvement. The example we use is the impact of couples therapy on affect dynamics over time, which is grounded in empirical research showing that partners’ affective states are dynamically interdependent, becoming synchronized through processes of mutual influence [@butler2013emotional]. Such coupling can either amplify or dampen emotional experiences, with implications for individual well-being and relationship stability. Using this context, we progressively build from basic assumptions of linear change to models incorporating feedback loops, random fluctuations, dyadic interdependence, and the effects of therapeutic inputs. Through this step-by-step approach, we introduce different models of affective processes that reflect increasing levels of complexity:

1. **Steady (Linear) Change**: Affect changes at a constant rate from its starting level between observations—the simplest step beyond a flat baseline. The same structure arises in many other settings, such as developmental change, treatment response, learning, or practice effects, whenever a simple model of systematic change over time is needed.
2. **Nonlinear Change (Discrete Time)**: The rate of change now depends on affect at the preceding observation, so trajectories can bend rather than grow in a straight line. The same logic applies, for example, to symptom recovery, skill acquisition, or fatigue when becomes easier or more difficult at different levels.
3. **Nonlinear Change with Random Fluctuations (Continuous Time)**: Change now unfolds continuously, with the rate depending on current (rather than past) affect. Random fluctuations represent unmodelled influences that perturb the process between observations, separating measurement error from process noise -- useful in any context where the observations do not perfectly reflect the underlying process of interest. This section include a conceptual introduction to continuous-time dynamic modelling.
4. **Couple Dynamics with Self and Partner Effects (Continuous Time)**: A multivariate extension in which each partner's rate of change depends on their own affect and their partner's. The same coupling or cross-effects structure could describe, among other cases, linked symptoms, cognition and physiology, parent–child dynamics, or interacting behaviors.
5. **Couple Dynamics with Individual Differences and Time-Independent Moderation (Continuous Time)**: People differ in their dynamic parameters, and a stable between-person variable (here, time spent together) moderates partner coupling. Such moderation questions arise when stable traits, diagnoses, treatment groups, age, or context relate to differences in dynamic parameters.
6. **Couple Dynamics with Individual Differences and Time-Dependent Moderation (Continuous Time)**: Partner coupling depends on whether the couple is communicating at that moment -- a time-varying moderator switches a dynamic parameter on or off. Similar logic applies whenever an observed time-varying condition modifies a dynamic parameter; for example, nicotine replacement may dull the link between certain behaviours or contexts and craving.
7. **Nonlinear Change with a Transient External Shock (Continuous Time)**: A therapy session delivers an instantaneous input when it occurs; the effect then propagates and dissipates through the dynamics. Transient inputs can likewise represent, for example, acute stressors, prompts, medication doses, feedback, or experimental manipulations.
8. **Nonlinear Change with a Persistent External Shock (Continuous Time)**: Therapy shifts the process parameters themselves, leaving a lasting change in regulation or level. Persistent inputs could also model, for example, learning, cumulative treatment response, habit formation, or enduring environmental change.

These different models provide researchers with a structured approach to modelling psychological dynamic, allowing for the representation of both complex within-person change and between-person heterogeneity. While it is not necessary for the reader to engage thoroughly with every modelling example, such modelling competencies are an important basis for considering the final, and in some sense most important section of the tutorial, addressing model evaluation in the dynamic systems context. In this section we discuss some necessary checks to ensure the fitted model is appropriate, and showcase a process of iterative model evaluation and improvement starting from a misspecified model and working toward a more appropriate representation of the data generating mechanism. For readers less familiar with dynamic systems modelling, skipping models 5 to 8 is suggested, while those more familiar may instead benefit from moving quickly over models 1 and 2, and noting that models 7 and 8 reflect relatively advanced topics that are not often used in psychological research at present. 
...
Here we see our linear model provides a good representation of the idea that Alex's affect increases at a steady pace each time we observe them. Albeit with some random fluctuations, which in this simple model we attribute to unsystematic variation rather than to changes in the underlying affect process.
...
slopeMean <- .5
slopeSD <- .2
zInitialAffect <- as.numeric(scale(t0Affect)) #initial affect in SD units
randomSlopePart <- rnorm(n = NSubjects) #extra individual differences in slopes
randomSlopePart <- as.numeric(scale(randomSlopePart)) #also in SD units
zTimeEffect <- #standardized slope with desired correlation to initial affect
  initSlopeCor * zInitialAffect + #correlated part
  sqrt(1 - initSlopeCor^2) * randomSlopePart #remaining random part
...
plot_trajectory(data, color_var = "InitialAffect", group_var = "Subject", show_legend = TRUE)
```
...
Beyond couple dynamics, the same logic applies whenever an observed time-varying condition may turn a dynamic relation on or off, or strengthen and weaken it, from one occasion to the next. A few examples are: stress exposure might allow symptoms to influence each other only during high-stress periods; medication status might change how quickly a process returns to baseline; poor sleep might increase inertia in mood from one day to the next; social contact might determine whether one person's state can affect another's rate of change; task condition might reveal cross-process coupling only in certain experimental phases; and environmental demands might alter growth or regulation parameters as workload shifts. In each case, the moderator is observed at the same time scale as the process, and it enters the model by scaling a dynamic parameter rather than by adding a direct impulse to the state (as with the therapy example earlier). This is not limited to cross-effects: time-dependent moderators can in principle act on auto-effects, intercepts, cross-effects, or other system parameters. A different extension, not developed in detail here but possible with `ctsem` using similar syntax, would be state-dependent moderation, where the strength of one process's effect depends on the current level of another modeled process -- for example, negative affect might influence anxiety only when arousal is already high. 
...
The trajectory-based posterior predictive check shows the model implied expectations (red) versus the empirical data (black) over time. Here it is quite clear that we are fitting a linear model to a nonlinear trajectory. Our model also seems to underestimate the variance at earlier time points while overestimating the variance at later observations. But how can we improve our model to better account for these observations?
