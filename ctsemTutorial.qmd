...
\sectionfullref{sec-transient-external-shock} introduces approaches to model interventions and external shocks and estimate their influence on dynamic processes. 
...
Readers less familiar with dynamic systems modelling may wish to focus on \sectionfullref{sec-modelling-univariate-growth}, \sectionfullref{sec-modelling-univariate-dynamic-systems}, and \sectionfullref{sec-model-evaluation}, leaving \sectionfullref{sec-modelling-multivariate-systems}, \sectionfullref{sec-individual-differences}, \sectionfullref{sec-transient-external-shock}, and \sectionfullref{sec-nonlinear-dynamics} for a later pass. More experienced readers may instead prefer to skip or skim \sectionfullref{sec-modelling-univariate-growth}, but consider that \sectionfullref{sec-nonlinear-dynamics} contains relatively advanced material. 
...
After fitting the model to our data, we can summarize the parameter estimates.
...
for(i in 1:Nobs){ # for each time point
  if(i==1) Affect[i] <- t0Affect #if first time point, initial affect
...
- **Standardised residual covariance**: Standardized residual (co)variance, characterising unexplained variation after the model's predictions have been accounted for.
...
From our summary table, we can see that the estimated coefficients are not exactly the same as the true values, due to sample variation in the data. The model does however capture the general trend of affect dynamics over time, with parameter estimates close to our specified generating model. The estimated state dependence coefficient is around -0.1, the continuous intercept (B) approximately 1, and the system noise standard deviation around 0.2. The random effects standard deviation for initial affect is approximately 2, and the residual standard deviation is around 0.05.
...
Now we can use the standardized residual variance estimate (in the **Standardised residual covariance** table at the top of the summary, equivalent to 1 - $R^2$, also accessible as `s$residCovStd`) to see how much of the variance in our empirical data our model has accounted for. We can see that our model explains a high proportion of the variance in our observations. This is fairly typical for repeated measures data, and doesn't imply that our model is appropriate, but provides a useful baseline for comparison.
...
A revised model which includes Alex and Robin's affect dynamics and their interdependence can be written with the same general equation as earlier, with only the dimensions of the equation components differing, e.g. the $A$ matrix is now a 2x2 matrix:

$$d\eta = (A\eta(t) + B)dt + GdW(t)$$

Or we can split the matrix equation two separate equations, one for each partner:
...
#| fig-cap: "Affect Trajectories for Both Partners in Two Couples."
NSubjects <- 20
...
@fig-couple-dynamics shows the affect trajectories for two couples. Each partner in the couple is clearly distinct, but couples may show similarities in comparison to other couples, due to both shared influences and effects from one partner on the other.
...
Checking and understanding model specification for multivariate systems can be much easier when looking at the full, expanded model equations in matrix form --- we can generate a LaTeX formatted representation of the equations we just specified by running the `ctModelLatex()` function.[^tinytex]
...
When specifying the system matrices in `ctsem`, they can be explicitly created using the `matrix()` function in R, but can also be passed in as a vector (except for the LAMBDA, factor-loading matrix) using the `c()` function, which will be read in **row-wise** in contrast to typical R convention. 

### Model Fitting

Now we fit our model and could, as earlier, use `summary` to view estimated parameters --- here we just demonstrate one way to obtain more easily
...
This time, we'll look at how well the model predicts our observations using every earlier data point (i.e., just as the model does it) for our 3rd dyad (subject 3). 

```{r plot-predictions-ctsem-multivariate, out.height='30%'}
#| label: fig-ctsem-multivariate-pred
#| fig-cap: "Predictions for Both Partners in Dyad 3 Conditional on Every Observation."
ctPredict(fit = f_couple, plot=TRUE, subjects = 3, 
  kalmanvec=c('y','yprior'))
```

@fig-ctsem-multivariate-pred shows a very different picture to the earlier linear models. Our model predictions show the complex interplay between affect processes of the two individuals in the dyad. The uncertainty in the predictions (shaded polygons) reflects both the system noise and measurement error. We could instead examine the latent process predictions and uncertainty by changing the `kalmanvec` argument to `'etaprior'` instead of `'yprior'`.

### Visualize Dynamics --- Independent Shocks

We can also visualize the model-implied dynamics (i.e.,auto-effect and cross-effect state dependencies), using an 'impulse response function' that essentially shows us the outcome of a hypothetical experiment. In this experiment, we take both Alex and Robin, place them in a controlled setting where no external factors impact their affect. Then, we intervene on Alex's affect to increase it by 1 at time 0, and plot how Alex's and Robin's affect change over time in response to this shock to Alex's affect. Examining @fig-impulse-response, we see that if we increase Alex's affect by 1 at time 0, it is expected to still be approximately 0.40 above baseline after 5 weeks (red solid line, `Affect1.Affect1` at time interval = 5) because of self-dependency and the bidirectional cross-effects between Alex and Robin. Robin's affect however doesn't jump initially, but rises more gradually in response to the higher affect of Alex --- it is expected to be 0.20 by week 5 (green solid line, `Affect2.Affect1` in @fig-impulse-response at time interval = 5). We can also visualize what happens if we increase Robin's affect by 1 *instead of Alex's* by viewing the blue and purple solid lines. In this case, where our hypothetical impulse generates a change of 1.0, the plotted values also correspond directly to the discrete-time coefficients implied by the model, for the times shown --- so with an interval of 5 weeks, Alex's affect exhibits an autoregression of 0.40, and so on. 
...
For our case, we are going to consider two sources of individual differences. First, we are going to assume that individuals have differences in their long-term, post therapy baseline affect. This will be done by allowing individual differences in the continuous intercept. Allowing for this makes our model a continuous-time analogue to the popular random intercept cross-lagged-panel model [@hamaker2015critiquea]. Second, we will assume that couples who have been together longer have a stronger influence on each others' affect. This will be incorporated by allowing for individual differences in the strength of the cross-effect state-dependence terms, and by allowing the size of each couple's cross-effect to depend on the average time they spend together.
...
#| fig-cap: "Affect Trajectories for Three Dyads, with Stable Individual Differences."
NSubjects <- 20
times <- seq(from=0, to=40, by=1) #time points of measurement
Nobs <- length(times) #number of observations per subject
t0Affect1 <- rnorm(n = NSubjects, mean = 5, sd = 2)
...
# Interventions and External Events {#sec-transient-external-shock}
...
In self-regulating systems, where inputs tend to dissipate back towards equilibrium, such an impulse will also gradually decay back toward the equilibrium. This provides a natural model for transient events whose effect is expected to be strongest when it occurs and then dissipate through the system dynamics.
...
The models we have considered so far have assumed that dynamic parameters are fixed over the observation window, but sometimes it is important to consider how the system parameters themselves may change over time. We provide two demonstrations of this. First, a latent state can moderate system parameters --- in our example, when partner affect transmits more strongly at higher levels of activation. Second, an external input can develop through a separate latent process with its own dynamics, allowing intervention effects that accumulate or persist differently from the main system, resulting in a persistent shift in system dynamics.   

## Cross Dynamics with Latent State Moderation {#sec-indiff-latentmod}

In \sectionfullref{sec-indiff-tipred}, we allowed partner coupling to depend on how much time a couple spends together, treated as a stable between-couple characteristic. We may also want coupling itself to depend on the partners' current affective states. For example, one partner's affect may transmit more strongly to the other when that partner is already highly activated than when affect is low --- a form of emotional contagion or amplification at higher levels of arousal. This is not moderation by an observed covariate, but by the current level of another latent process in the system, sometimes termed a latent interaction.
...
To generate data with latent state moderation of cross-effect coupling, we allow each partner's cross-effect to depend on the other partner's current affect level. The cross-effect for partner 1 becomes `cross12 <- cross12base + cross12mod*inv_logit(Affect2/10)`, and symmetrically for partner 2. When `cross12mod` is positive, influence from one partner strengthens as the other partner's affect increases. We use the inverse logit function on a scaled affect variable to ensure that change due to moderation is limited, such that extreme lows and extreme highs do not moderate substantially more than moderate lows and highs. 
...
## System Moderation Via Persistent Intervention {#sec-persistent-external-shock}

Combining the ideas from \sectionfullref{#sec-transient-external-shock} and \sectionfullref{sec-indiff-latentmod}, here we consider a) a shock to the system that *does not* dissipate, and b) does not directly impact the latent processes but instead changes the system parameters. So rather than assuming an impulse effect transmits through time with the same dynamics as the main system, we can allow the persistence, or 'shape' of the effect to differ from the rest of the system. This approach can be generalized to achieve things such as separated trends and dynamics processes [@driver2023formalizing], cross-effects that operate at different time scales [@haehner2025dynamics], oscillatory or more complex dynamic patterns [@voelkle2013continuousa], and more.
