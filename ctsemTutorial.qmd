...
impact-statement: |-
  This tutorial shows how psychological researchers can use R and the ctsem package to build and understand dynamic systems models while keeping the modelling assumptions visible. By working from theory to hand-coded simulations, estimation, checking, and revision, we aim to facilitate the translation of theoretical concepts into empirical models and improve understanding of psychological processes and their interrelations over time.
...
1. **Steady (Linear) Change**: an affect process that changes at a steady rate from an initial state of affect during each observation. Similar models appear in developmental, treatment, learning, and practice-effect contexts when the first question is whether there is systematic change over time.
2. **Nonlinear Change (Discrete Time)**: an affect process whose growth rate depends on a person's level of affect at the observation before the change happens--creating a nonlinear growth process. The same idea is relevant to symptom recovery, skill acquisition, fatigue, and other processes where change slows or accelerates depending on the current level.
3. **Nonlinear Change with Random Fluctuations (Continuous Time)**: an affect process that changes continuously over time, and whose rate of change depends on a person's level of affect at the moment of change--creating a nonlinear growth process. Here we include randomness in the process, reflecting unmodelled inputs that generate fluctuations in affect. Similar distinctions between systematic regulation and process noise arise in stress recovery, psychophysiology, ecological momentary assessment, and symptom dynamics. The conceptual shift necessary to understand modelling dynamics in continuous time is explained prior to the introduction of this model.
4. **Couple Dynamics with Self and Partner Effects (Continuous Time)**: moving to multivariate models, an affect process for a couple whose rate of change is dependent on their own level of affect and their partner's level of affect at a given moment. The same structure can represent links among symptoms, cognition and physiology, parent-child dynamics, or interacting behavioral states.
5. **Couple Dynamics with Individual Differences and Time-Independent Moderation (Continuous Time)**: in this model, we introduce individual differences in system dynamics and a time-independent moderator (i.e. partner effects are moderated by the overall time they spend together). Similar moderation questions arise when stable traits, diagnoses, treatment groups, age, or context explain why people differ in dynamic parameters.
6. **Couple Dynamics with Individual Differences and Time-Dependent Moderation (Continuous Time)**: in this model, the partner effects are moderated by a time-varying moderator tracking whether the couple is together (time-dependent covariate). More generally, dynamic relations may depend on momentary stress, medication use, social context, sleep, task condition, or environmental exposure.
7. **Nonlinear Change with a Transient External Shock (Continuous Time)**: here the affect process is impacted by a therapy session that only directly impacts affect during the session. Transient inputs can also represent acute stressors, prompts, medication doses, feedback, or experimentally delivered events.
8. **Nonlinear Change with a Persistent External Shock (Continuous Time)**: here the affect process is impacted by a therapy session and the therapy session induces persistent changes in the process. Persistent inputs can represent learning, treatment response, habit formation, cumulative exposure, or lasting environmental changes.
...
Here we see our linear model provides a good representation of the idea that Alex's affect increases at a steady pace each time we observe them. Albeit with some random fluctuations, which in this simple model we attribute to measurement error rather than to changes in the underlying affect process.
...
plot_trajectory(data, color_var = "InitialAffect", show_legend = TRUE)
```
...
In the second prediction plot, we visualize the model's predictions after it has learned from all the observations in our dataset. So, data from past, present, and future observations are used by the model to predict each observation. Here the model-implied trajectory does an excellent job at mapping onto our observations --- having learnt the individual difference parameters needed for this subject specifically, predictions are far more accurate.
...
To formally represent continuously changing processes we need to use differential equations. But even without a technical understanding of differential equations researchers can form an intuitive understanding of continuous processes and apply them to their own substantive domain. Our intuition can be aided by building a mental image of a continuously changing process. One way to soften the move from discrete to continuous time is to think of a difference equation, where change is computed over a finite interval, and then make that interval arbitrarily small. In the limit, the left-hand side no longer describes change from one observation to the next, but the instantaneous rate of change.
...
We can model random fluctuations in the affect process by adding a system noise term to our differential equation. System noise represents unmodelled inputs to the latent process at the chosen level of abstraction. Unlike measurement error, which affects only the observation, system noise changes the latent state and is propagated forward by the dynamics. With our current equation, the rate of change in affect $\frac{d\eta}{dt}$ is purely deterministic, meaning that how fast affect changes at a given moment is entirely determined by the terms in the right-hand side of the differential equation ($A\eta(t) + B$).

To model the impact of random influences on Alex's affect trajectory, we want to add a term that introduces a random "kick" to the rate of change at each moment. Meaning that aside from the deterministic effects there will be some random ups and downs in Alex's growth rate. To add a "kick" to Alex's rate of change we can introduce a noise term that is sampled from a normal distribution. This turns our (ordinary) differential equation into a stochastic differential equation [@KloedenPlaten1992], with stochastic denoting the random noise component of our continuous process.
...
Our `ctsem` model can be thought of as linking a latent (unobserved) 'true' affect process to observed (or 'manifest') variables, which are part of our data. Observed variables are treated as indicators of the underlying latent process, and are linked to the latent states via a measurement model. For the running example, this means that Alex's affect is assumed to evolve continuously between observations, while the observed app ratings are noisy snapshots of that underlying process.
...
@fig-ctsem-basic-model brings together the pieces introduced above. The subject-level distribution describes individual differences in initial affect. The dynamic model describes how the latent affect state changes through drift, continuous intercept, and diffusion. The measurement model describes how the latent state becomes the observed affect score plus measurement error. Reading the model in this order can help avoid treating the `ctsem` syntax as a black box.
...
- **`$residCovStd`**: Standardized residual covariance, useful as one diagnostic of remaining model-implied prediction error
...
We can also use visualizations to build intuition about the model-implied dynamics (i.e. auto-effect and cross-effect state dependencies). To do this, we can create a plot that tracks the results of an imaginary experiment. In this experiment, we take both Alex and Robin, and place them in a completely controlled setting, where no external factors can impact their affect. That is, their affect remains at baseline unless we do something about it. Second, we intervene on Alex's affect to increase it by 1 at time 0. Third, we plot how Alex's and Robin's affect would be expected to change over 10 weeks following the shock to Alex, as a function of their individual auto-effects and the cross-effects they have on each other. For example, if we increase Alex's affect by 1 at time 0, Alex's affect is expected to still be approximately 0.40 above baseline after 5 weeks (red solid line, `Affect1.Affect1` in the plot at time interval = 5) because of self-dependency and the bidirectional cross-effects between Alex and Robin. Instead, Robin's affect doesn't jump initially, but rises more gradually in response to the higher affect of Alex --- it is expected to be 0.20 by week 5 (green solid line, `Affect2.Affect1` in the plot at time interval = 5). We can also visualize what happens if we increase Robin's affect by 1 *instead of Alex's* by viewing the blue and purple solid lines. This plot is known as an impulse response function plot [@Lutkepohl2005].
...
Stable moderators are useful whenever theory suggests that people or systems differ in their dynamic parameters because of relatively enduring characteristics. In other psychological applications, these moderators might be diagnostic group, age, trait affect, baseline stress, treatment condition, relationship length, socioeconomic context, or a stable environmental exposure. The key question is whether the moderator plausibly changes the dynamic process itself, rather than merely predicting average levels.
...
We may feel uncomfortable relying on people's retrospective estimate of how much time they spend together to moderate their degree of interdependence. However, we also do not want to assume that couples continuously affect each other even when they are not in each other's presence. For instance, according to our model, if Alex gets fired, Alex's change in affect would immediately influence Robin's affect, even if Robin is completely unaware of this event. If we have information about Alex and Robin's proximity, we can use this information to tell our model to only allow Alex's affect to impact Robin's rate of change when the couple is together. This can be done by moderating our couple's cross-effect parameter by a time-dependent moderator of presence.

This idea generalizes to momentary contexts such as stress exposure, medication status, sleep quality, social contact, task condition, or environmental demands. These are observed moderators of a dynamic parameter. A different extension, not developed in detail here, would be state-dependent moderation, where the strength of one process's effect depends on the current level of another modeled process. Such latent-state interactions can be useful, but require stronger theory and usually more information in the data.
...
We can plot the smoothed estimates of the latent variable states for the first three subjects in our dataset, which shows the increase in the accumulated therapy at the time of the intervention, which causes increases in affect over time. Here we can see the instantaneous impulse of the therapy session on accumulated therapy, and the long term growth in affect due to accumulated therapy.
...
```
The density-based posterior predictive check compares the empirical distribution of the observed variables and likelihood contributions with distributions generated from the fitted model. It is difficult to get a clear sense of exactly where things are imperfect from such a plot, but this aggregate view provides another approach to see that there clearly are some patterns in the data that deviate from the model's expectations. Perfect overlap between the empirical and model-implied distributions for each variable doesn't necessarily mean the model is appropriate, but does at least indicate that the overall distributions are similar.
...
The trajectory-based posterior predictive check shows the model implied expectations (red) versus the empirical expectations (black) over time. Here it is quite clear that we are fitting a linear model to a nonlinear trajectory. Our model also seems to underestimate the variance at earlier time points while overestimating the variance at later observations. But how can we improve our model to better account for these observations?

Taking a closer look at the observed individual growth trajectories (fainter black lines), we can see there seems to be a dependency between people's initial affect values and their rate of change in affect. That is, people who start with very high affect values have an almost flat growth curve, while people with a low initial level have a very steep growth curve which slowly plateaus as they reach higher values of affect. Thus, it seems that we should add a state-dependency parameter to our model to capture this relationship. In empirical applications, this step should be justified by theory and diagnostics together: state dependence is most defensible when researchers expect the current state to regulate subsequent change, and when model-implied behavior improves in ways that match the substantive process rather than merely increasing flexibility.
...
Now we can again see if adding this state-dependence term has helped us explain more variance in our data by considering the $R^2$ --- and indeed, we can see that we have around 10% residual variance, or 90% explained variation in our data --- noticeably more than before. This is encouraging, but it is not sufficient: the question is whether the model-implied behavior and residual structure are more appropriate. So now we can go view our autocorrelation plot to check if there is still information left in the residuals that our model could explain.
...
Overall, we can see that fitting the model to the data it generated leads to a greater $R^2$, a near-zero autocorrelation between residuals, and model-implied lagged correlations that map well onto the observed lagged correlations.
...
Dynamic state-space models can fail in ways that are informative. Common warning signs include nonconvergence, Hessian warnings or unreliable standard errors, unstable estimates across small changes in specification, weakly identified parameters, boundary variances or correlations, implausibly fast or slow dynamics, strong sensitivity to start values or priors, and poor posterior predictive behavior. These problems can arise from the data, the model, or their mismatch. For example, variables with little or no variance, sparse observations, low reliability, an unsuitable time unit, or too many random effects can all make estimation fragile.

Several practical checks are useful before interpreting a difficult fit. First, inspect the data scale and the time scale. Continuous-time models are often easier to estimate when a time unit of 1 represents a moderate amount of change; very large or very tiny intervals can produce numerical problems or dynamics that are hard to identify. Centering or scaling observed variables and moderators can also help make start values and unconstrained parameter scales more reasonable. Second, inspect the expanded model specification to ensure that defaults have not introduced unintended free parameters or random effects. Third, fit simpler intermediate models and add complexity only when the previous model behaves sensibly. Fourth, check whether changing reasonable start values, priors, time units, or constraints changes the substantive conclusion. For Bayesian estimation, also inspect chain mixing, $\hat{R}$, effective sample size, divergent transitions, and trace plots.

Finally, treat warnings as prompts for model criticism rather than as purely technical obstacles. A Hessian warning, boundary estimate, or sensitivity to priors may indicate that the data do not identify the model, that the time scale is poorly chosen, or that a theoretically distinct influence is being forced into the wrong parameter. The appropriate response may be to rescale, simplify, collect denser or more reliable data, or revise the substantive model.

## Extensions Not Developed Here

The examples above are intentionally limited. Two extensions are worth noting because they follow naturally from the models we have specified. First, one could allow state-dependent moderation, where the effect of one process on another depends on the current level of one or both processes. For example, stress might increase symptom coupling only when arousal is already high, or social support might buffer negative affect only below a certain level of distress. Second, one could use nonlinear measurement models when the observed scale changes sensitivity across the latent range, such as bounded, skewed, or thresholded outcomes generated by a smoother latent process. These extensions can be useful, but they require stronger theory, richer data, and additional checks beyond the introductory workflow shown here.
...
The practical goal is thus iterative and cautious model building. Researchers should begin with explicit theoretical assumptions, simulate the implications of those assumptions where possible, fit models that are no more complex than the theory and data can support, check whether the model-implied behavior is plausible, and simplify or revise when the model fails. We hope this tutorial helps researchers use dynamic systems models as transparent tools for theory development rather than as black-box descriptions of longitudinal data.
