...
author:
  ...
  - name: Michael Aristodemou
    corresponding: false  # Change to true if he should be listed as a corresponding author
...
This model is commonly termed a linear mixed-effects model [@hoffman2022catching], because it includes both fixed effects (parameters that are assumed to be the same across all subjects, often interpreted as population-level effects) and random effects (parameters that allow for individual variations, such as subject-specific intercepts $\eta_{t0i}$ or slopes $B_i t$).
