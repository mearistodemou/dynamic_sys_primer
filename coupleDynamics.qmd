...
1. The `DRIFT` option specifies the state dependence term $A\eta(t)$, which we call `stateDependence`, making the rate of growth depend on the current level of affect.
...
3. The `DIFFUSION` option specifies the system noise term ($GWd(t)$), which we call `systemNoise`, allowing the model to capture random fluctuations around the model-implied growth trajectory.
...
\left(\underbrace{\begin{pmatrix} A_1 & A_{cross_{12}}\\ A_{cross_{21}} & A_2  
\end{pmatrix}}_{\text{Drift}}

\underbrace{\begin{pmatrix} \eta_1 \\ \eta_2 
\end{pmatrix}}_{{\eta(t)}} (t) + 

\underbrace{\begin{pmatrix} B_1 \\ B_2 
\end{pmatrix}}_{\text{CINT}}\right) dt +

\underbrace{\begin{pmatrix} G_{11} & G_{12} \\ G_{21} & G_{22}
\end{pmatrix}}_{\text{Diffusion}} 

\underbrace{d \begin{pmatrix} W_1 \\ W_2
\end{pmatrix} (t) }_{\text{dW(t)}}
\end{aligned}
$$
...
Our hypothetical experiment with independent shocks, may have low generalizability to real world dynamic systems. This is because often a shock that happens to one partner can also affect the other partner (see [https://doi.org/10.31234/osf.io/y3e9d](https://doi.org/10.31234/osf.io/y3e9d), for more on the combined interpretation of system noise and dynamics). 
