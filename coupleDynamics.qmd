...
author:
  - name: Charles C. Driver
    ...
    roles:  # pick what really applies
      - conceptualization
      - methodology
      - software
      - validation
...
word-count: false          # leave true if journal asks for automatic word count
floatsintext: true
...
no-ampersand-parenthetical: false     # keep APA style ampersands in () citations
...
format:
  ...
  apaquarto-docx:
    toc: false
  apaquarto-pdf:
    documentmode: doc    # journal (two-column) mode; use man for manuscript
    font-size: 10pt
    a4paper: true
    keep-tex: true       # drop when production‐ready
...
#create a tempdir for tex files and track it's location
dir.create('textemp',showWarnings = FALSE)
texdir='./textemp'
```
